####create def for fsc
def_str<-c("/home/mae47/rds/hpc-work/RAD_seq/D_trimaculatus/NRS_DGA/simulations/cosmo_model/island_1/model/template/input_autosome.def") 
...
#All times are in generations: Dt 1 gen = 3 years
for (y in 1:20){  #no. sims per folder
	...
	break }	
    	...
    	nbott<-runif(1,4,nanc) #10k to nanc
	npop_rs<-runif(1,nbott,5.5) #nbott to ~300k

	
	trec<-runif(1,3833,5000) #time growth starts 11500ybp to 15000ybp
	tlengmax<-trec-1 #ensures tstop > 0
	tleng<-runif(1,50,tlengmax) #length of growth/growth period 150y to trec
	tstop<-trec-tleng #end of growth
	

tbott<-runif(1,8000,12000) #24000-36000y
tanc<-runif(1,4.125,5.301) #log, 13333-200000gens ie ~40000-600000y
mig<-runif(1,-6.51,-2.98) #prop per gen, log, 0.0000003-0.00105gens ie 0.0000001 - 0.00035 per year or 0.00001% - 0.035%
