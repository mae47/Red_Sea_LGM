####create def for fsc
def_str<-c("/home/mae47/rds/hpc-work/RAD_seq/P_maculosus/simulations/cosmo_model/island_1/model/template/input_autosome.def") 
...
#All times are in generations: Pm 1 gen = 4 years
for (y in 1:10){  #no. sims per folder
	...
	trec<-runif(1,2875,3750) #time growth starts ~11500ybp to 15000ybp
	tlengmax<-trec-1 #ensures tstop > 0
	tleng<-runif(1,38,tlengmax) #length of growth/growth period ~150y to trec
	tstop<-trec-tleng #end of growth
	

tbott<-runif(1,6000,9000) #24000-36000y
tanc<-runif(1,4,5.176) #log, 10000-150000gens ie 40000-600000 years
mig<-runif(1,-6.40,-2.85) #prop per gen, log, 0.0000004-0.0014 per gen, ie 0.0000001 - 0.00035 per year or 0.00001% - 0.035%
