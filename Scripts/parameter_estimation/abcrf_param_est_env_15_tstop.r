...
one_param_input<-data.frame(c=sim_params$tstop,sim_summary_stats)
...
tstop_param_rf <- one_param_rf
predict_output_tstop <- predict(one_param_rf,obs=obs_summary_stats,training = one_param_input)
rm(one_param_rf)
#save environment
save.image(paste0(base_path,"/analysis/abcrf/environments/tstop_param_rf_11092023.RData"))
