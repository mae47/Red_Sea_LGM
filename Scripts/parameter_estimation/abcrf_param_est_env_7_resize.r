...
one_param_input<-data.frame(c=sim_params$resize,sim_summary_stats)
...
resize_param_rf <- one_param_rf
predict_output_resize <- predict(one_param_rf,obs=obs_summary_stats,training = one_param_input)
rm(one_param_rf)
#save environment
save.image(paste0(base_path,"/analysis/abcrf/environments/resize_param_rf_11092023.RData"))
